...
from .jsonschema_adapter import JsonSchemaAdapter, supported_json_schema_claim
...
def build_json_schema_package(
    *,
    ...
    ir = parser.parse_ir(
        ...
        approved_by="phase15@example.invalid",
    ...
    evidence = _json_schema_evidence_for_ir(
        bound_ir,
        ir_hash=ir_hash,
        adapter=adapter,
    ...
    write_json(output_dir / "review.json", _review(bound_ir, reviewer="phase15@example.invalid"))
    ...
    (output_dir / "implementation-spec.md").write_text(
        _implementation_spec(
            ...
            evidence_lines=_json_schema_evidence_lines(adapter, tasks),
        )
    )
...
def validate_json_schema_package(
    package_dir: Path, adapter: JsonSchemaAdapter
    ...
    _validate_json_schema_package_integrity(
        package_dir,
        adapter,
        ir,
        bindings,
...
def _validate_json_schema_package_integrity(
    package_dir: Path,
    adapter: JsonSchemaAdapter,
    ir: RequirementIR,
    ...
    expected_evidence = _json_schema_evidence_for_ir(
        ir,
        ir_hash=ir_hash,
        adapter=adapter,
        missing_symbols=binding.missing_symbols,
    ...
    _expect(binding.bound_ir.bindings == ir.bindings, "bindings.json does not match JSON Schema adapter")
    _expect(assumptions.root == ir.assumptions, "assumptions.json does not match requirement.ir.json")
    _expect(
        tasks.root == expected_tasks,
        "verification-tasks.json does not match requirement.ir.json or JSON Schema hash",
    )
    _expect(generated_tests.root == [], "generated-tests.json is not supported for JSON Schema packages yet")
    _expect(counterexamples.root == [], "counterexamples.json is not supported for JSON Schema packages yet")
    _expect(normalized_traces.root == [], "normalized-traces.json is not supported for JSON Schema packages yet")
    _expect(
    ...
    _expect(
        ...
        == _implementation_spec(
            ...
            status.status.value,
            adapter_id=adapter.adapter_id,
            evidence_lines=_json_schema_evidence_lines(adapter, expected_tasks),
        ),
...
def _json_schema_evidence_for_ir(
    ir: RequirementIR,
    *,
    ir_hash: str,
    adapter: JsonSchemaAdapter,
    ...
    unsupported_claims = [] if supported_json_schema_claim(ir) else [ir.claim.kind]
    ...
    claims = [
        EvidenceClaim(
            id="C-static",
            description="Symbols are statically resolved by the JSON Schema adapter.",
    ...
    for task in tasks:
        claim = _claim_for_json_schema_task(task, task_results_by_id.get(task.id))
        claims.append(claim)
...
def _claim_for_json_schema_task(task: VerificationTask, result: BackendResult | None) -> EvidenceClaim:
    required = EvidenceLevel.TYPE_CHECKED
...
def _json_schema_evidence_lines(
    adapter: JsonSchemaAdapter, tasks: list[VerificationTask]
) -> list[str]:
    return [
        "IR type-checked.",
        ...
        "JSON Schema symbol bindings type-checked.",
        *(
            ["JSON Schema state value declarations type-checked."]
            if any(task.payload.get("task") == "state_value" for task in tasks)
            else []
        ),
        *(
            ["JSON Schema numeric delta metadata type-checked."]
            if any(task.payload.get("task") == "numeric_delta" for task in tasks)
            else []
        ),
    ]
