...
from .graphql_adapter import GraphQlAdapter, supported_graphql_claim
...
def build_graphql_package(
    *,
    ...
    ir = parser.parse_ir(
        ...
        approved_by="phase14@example.invalid",
    ...
    evidence = _graphql_evidence_for_ir(
        bound_ir,
        ir_hash=ir_hash,
        adapter=adapter,
    ...
    write_json(output_dir / "review.json", _review(bound_ir, reviewer="phase14@example.invalid"))
    ...
    (output_dir / "implementation-spec.md").write_text(
        _implementation_spec(
            ...
            evidence_lines=_graphql_evidence_lines(adapter, tasks),
        )
    )
...
def validate_graphql_package(
    package_dir: Path, adapter: GraphQlAdapter
    ...
    _validate_graphql_package_integrity(
        package_dir,
        adapter,
        ir,
        bindings,
...
def _validate_graphql_package_integrity(
    package_dir: Path,
    adapter: GraphQlAdapter,
    ir: RequirementIR,
    ...
    expected_evidence = _graphql_evidence_for_ir(
        ir,
        ir_hash=ir_hash,
        adapter=adapter,
        missing_symbols=binding.missing_symbols,
    ...
    _expect(binding.bound_ir.bindings == ir.bindings, "bindings.json does not match GraphQL adapter")
    _expect(assumptions.root == ir.assumptions, "assumptions.json does not match requirement.ir.json")
    _expect(
        tasks.root == expected_tasks,
        "verification-tasks.json does not match requirement.ir.json or GraphQL schema hash",
    )
    _expect(generated_tests.root == [], "generated-tests.json is not supported for GraphQL packages yet")
    _expect(counterexamples.root == [], "counterexamples.json is not supported for GraphQL packages yet")
    _expect(normalized_traces.root == [], "normalized-traces.json is not supported for GraphQL packages yet")
    _expect(
    ...
    _expect(
        ...
        == _implementation_spec(
            ...
            status.status.value,
            adapter_id=adapter.adapter_id,
            evidence_lines=_graphql_evidence_lines(adapter, expected_tasks),
        ),
...
def _graphql_evidence_for_ir(
    ...
    *,
    ...
    unsupported_claims = [] if supported_graphql_claim(ir) else [ir.claim.kind]
    ...
    claims = [
        EvidenceClaim(
            id="C-static",
            description="Symbols are statically resolved by the GraphQL adapter.",
    ...
    for task in tasks:
        claim = _claim_for_graphql_task(task, task_results_by_id.get(task.id))
        claims.append(claim)
...
def _claim_for_graphql_task(task: VerificationTask, result: BackendResult | None) -> EvidenceClaim:
...
def _graphql_evidence_lines(adapter: GraphQlAdapter, tasks: list[VerificationTask]) -> list[str]:
    return [
        "IR type-checked.",
        f"Symbols resolved through {adapter.adapter_id} adapter.",
        "Self-consistency checked.",
        "Supported claim shape SMT-checked.",
        "GraphQL symbol bindings type-checked.",
        *(
            ["GraphQL auth and state-change directives statically checked."]
            if any(task.payload.get("task") == "auth_rejection" for task in tasks)
            else []
        ),
        *(
            ["GraphQL operation return shape statically checked."]
