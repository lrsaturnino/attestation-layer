...
from .asyncapi_adapter import AsyncApiAdapter, supported_asyncapi_claim
...
def build_asyncapi_package(
    *,
    ...
    ir = parser.parse_ir(
        ...
        approved_by="phase16@example.invalid",
    ...
    evidence = _asyncapi_evidence_for_ir(
        bound_ir,
        ir_hash=ir_hash,
        adapter=adapter,
    ...
    write_json(output_dir / "review.json", _review(bound_ir, reviewer="phase16@example.invalid"))
    ...
    (output_dir / "implementation-spec.md").write_text(
        _implementation_spec(
            ...
            evidence_lines=_asyncapi_evidence_lines(adapter, tasks),
        )
    )
...
def validate_asyncapi_package(
    package_dir: Path, adapter: AsyncApiAdapter
    ...
    _validate_asyncapi_package_integrity(
        package_dir,
        adapter,
        ir,
        bindings,
...
def _validate_asyncapi_package_integrity(
    package_dir: Path,
    adapter: AsyncApiAdapter,
    ir: RequirementIR,
    ...
    expected_evidence = _asyncapi_evidence_for_ir(
        ir,
        ir_hash=ir_hash,
        adapter=adapter,
        missing_symbols=binding.missing_symbols,
    ...
    _expect(binding.bound_ir.bindings == ir.bindings, "bindings.json does not match AsyncAPI adapter")
    _expect(assumptions.root == ir.assumptions, "assumptions.json does not match requirement.ir.json")
    _expect(
        tasks.root == expected_tasks,
        "verification-tasks.json does not match requirement.ir.json or AsyncAPI document hash",
    )
    _expect(generated_tests.root == [], "generated-tests.json is not supported for AsyncAPI packages yet")
    _expect(counterexamples.root == [], "counterexamples.json is not supported for AsyncAPI packages yet")
    _expect(normalized_traces.root == [], "normalized-traces.json is not supported for AsyncAPI packages yet")
    _expect(
    ...
    _expect(
        ...
        == _implementation_spec(
            ...
            status.status.value,
            adapter_id=adapter.adapter_id,
            evidence_lines=_asyncapi_evidence_lines(adapter, expected_tasks),
        ),
...
def _asyncapi_evidence_for_ir(
    ...
    *,
    ...
    unsupported_claims = [] if supported_asyncapi_claim(ir) else [ir.claim.kind]
    ...
    claims = [
        EvidenceClaim(
            id="C-static",
            description="Symbols are statically resolved by the AsyncAPI adapter.",
    ...
    for task in tasks:
        claim = _claim_for_asyncapi_task(task, task_results_by_id.get(task.id))
        claims.append(claim)
...
def _claim_for_asyncapi_task(task: VerificationTask, result: BackendResult | None) -> EvidenceClaim:
...
def _asyncapi_evidence_lines(
    adapter: AsyncApiAdapter, tasks: list[VerificationTask]
) -> list[str]:
    return [
        "IR type-checked.",
        ...
        "AsyncAPI symbol bindings type-checked.",
        *(
            ["AsyncAPI event emission declarations type-checked."]
            if any(task.payload.get("task") == "event_emission" for task in tasks)
            else []
        ),
    ]
