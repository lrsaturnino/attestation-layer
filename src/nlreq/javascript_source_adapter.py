...
DEFINITION_PATTERN = re.compile(
    r"^(?:export\s+)?(?:async\s+)?function\s+(?P<function>[A-Za-z_$][\w$]*)\s*\("
    r"|^(?:export\s+)?(?:const|let|var)\s+(?P<const>[A-Za-z_$][\w$]*)\s*="
    r"|^(?:export\s+)?class\s+(?P<class>[A-Za-z_$][\w$]*)\b",
    re.MULTILINE,
)
...
class JavaScriptSourceLanguageAdapter(SourceLanguageAdapter):
    ...
    def resolve_symbol(self, ref: SymbolRef, manifest: SourceManifest) -> SourceSymbolResolution:
        ...
        for module in manifest.modules:
            ...
            if not path.is_file():
                continue
            for definition in _definitions(path).values():
        ...
        if len(matches) > 1:
            return SourceSymbolResolution(
                ...
                status="ambiguous",
                symbols=matches,
                reason="symbol appears in multiple JavaScript modules",
            )
...
def _definitions(path: Path) -> dict[str, JavaScriptDefinition]:
    text = path.read_text()
    matches = list(DEFINITION_PATTERN.finditer(text))
    definitions: dict[str, JavaScriptDefinition] = {}
    for index, match in enumerate(matches):
        name = match.group("function") or match.group("const") or match.group("class")
        if not name:
            continue
        start = match.start()
        next_start = matches[index + 1].start() if index + 1 < len(matches) else len(text)
        definitions[name] = JavaScriptDefinition(
            name=name,
...
def _span_for_definition(path: Path, definition: JavaScriptDefinition) -> SourceSpan:
    text = path.read_text()
    return SourceSpan(
...
def _snippet_for_definition(path: Path, definition: JavaScriptDefinition) -> str:
    text = path.read_text()
