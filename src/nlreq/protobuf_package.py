...
def build_protobuf_package(
    *,
    ...
    ir = parser.parse_ir(
        ...
        approved_by="phase17@example.invalid",
    ...
    evidence = _protobuf_evidence_for_ir(
        bound_ir,
        ir_hash=ir_hash,
        adapter=adapter,
    ...
    write_json(output_dir / "review.json", _review(bound_ir, reviewer="phase17@example.invalid"))
    ...
    (output_dir / "implementation-spec.md").write_text(
        _implementation_spec(
            ...
            evidence_lines=_protobuf_evidence_lines(adapter, tasks),
        )
    )
...
def validate_protobuf_package(
    package_dir: Path, adapter: ProtobufAdapter
    ...
    _validate_protobuf_package_integrity(
        package_dir,
        adapter,
        ir,
        bindings,
...
def _validate_protobuf_package_integrity(
    package_dir: Path,
    adapter: ProtobufAdapter,
    ir: RequirementIR,
    ...
    expected_evidence = _protobuf_evidence_for_ir(
        ir,
        ir_hash=ir_hash,
        adapter=adapter,
        missing_symbols=binding.missing_symbols,
    ...
    _expect(binding.bound_ir.bindings == ir.bindings, "bindings.json does not match Protobuf adapter")
    _expect(assumptions.root == ir.assumptions, "assumptions.json does not match requirement.ir.json")
    _expect(
        tasks.root == expected_tasks,
        "verification-tasks.json does not match requirement.ir.json or Protobuf schema hash",
    )
    _expect(generated_tests.root == [], "generated-tests.json is not supported for Protobuf packages yet")
    _expect(counterexamples.root == [], "counterexamples.json is not supported for Protobuf packages yet")
    _expect(normalized_traces.root == [], "normalized-traces.json is not supported for Protobuf packages yet")
    _expect(
    ...
    _expect(
        ...
        == _implementation_spec(
            ...
            status.status.value,
            adapter_id=adapter.adapter_id,
            evidence_lines=_protobuf_evidence_lines(adapter, expected_tasks),
        ),
...
def _protobuf_evidence_for_ir(
    ...
    *,
    ...
    unsupported_claims = [] if supported_protobuf_claim(ir) else [ir.claim.kind]
    ...
    claims = [
        EvidenceClaim(
            id="C-static",
            description="Symbols are statically resolved by the Protobuf/gRPC adapter.",
    ...
    for task in tasks:
        claim = _claim_for_protobuf_task(task, task_results_by_id.get(task.id))
        claims.append(claim)
...
def _claim_for_protobuf_task(task: VerificationTask, result: BackendResult | None) -> EvidenceClaim:
...
def _protobuf_evidence_lines(adapter: ProtobufAdapter, tasks: list[VerificationTask]) -> list[str]:
    return [
        "IR type-checked.",
        f"Symbols resolved through {adapter.adapter_id} adapter.",
        "Self-consistency checked.",
        "Supported claim shape SMT-checked.",
        "Protobuf/gRPC symbol bindings type-checked.",
        *(
            ["Protobuf/gRPC auth and state-transition options statically checked."]
            if any(task.payload.get("task") == "auth_rejection" for task in tasks)
            else []
        ),
        *(
            ["Protobuf/gRPC RPC response declaration statically checked."]
