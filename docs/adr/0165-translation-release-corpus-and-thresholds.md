...
Phase 156 is part of milestone 15, Semantic Intake And Translation Evidence. The roadmap requires closure for public semantic accuracy and false-acceptance bar without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: translation_release_corpus, semantic_label_set, threshold_policy, public_translation_benchmark_report. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
