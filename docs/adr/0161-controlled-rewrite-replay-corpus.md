...
Phase 152 is part of milestone 15, Semantic Intake And Translation Evidence. The roadmap requires closure for auditable NL-to-controlled rewrite evidence without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: rewrite_corpus, replay_report, rewrite_diff_report, manual_rewrite_metadata. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
