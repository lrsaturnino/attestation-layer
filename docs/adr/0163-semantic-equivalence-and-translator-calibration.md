...
Phase 154 is part of milestone 15, Semantic Intake And Translation Evidence. The roadmap requires closure for semantic agreement beyond structural equality without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: translator_candidates, semantic_equivalence_report, calibration_metrics, reviewer_override_policy. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
