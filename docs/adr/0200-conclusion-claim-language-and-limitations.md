...
Phase 191 is part of milestone 20, External Review And Conclusion Publication. The roadmap requires closure for public non-overclaiming statement without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: conclusion_claim_document, limitation_matrix, evidence_label_glossary, reviewer_signoff. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
