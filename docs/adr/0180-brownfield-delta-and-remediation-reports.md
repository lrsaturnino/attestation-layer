...
Phase 171 is part of milestone 17, Brownfield Spec Grounding And Drift Closure. The roadmap requires closure for actionable spec/code/test deltas without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: brownfield_delta_report, remediation_plan_artifact, pr_annotation_export, ownership_report. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
