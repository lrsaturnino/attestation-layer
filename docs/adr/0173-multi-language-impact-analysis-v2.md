...
Phase 164 is part of milestone 17, Brownfield Spec Grounding And Drift Closure. The roadmap requires closure for affected system area discovery without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: impact_report_v2, symbol_resolution_report, call_graph_report, impact_disagreement_report. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
