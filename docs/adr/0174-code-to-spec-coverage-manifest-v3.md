...
Phase 165 is part of milestone 17, Brownfield Spec Grounding And Drift Closure. The roadmap requires closure for precise coverage propagation and blockers without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: coverage_manifest_v3, coverage_gate_report_v3, coverage_gap_report, candidate_stale_blocker_report. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
