...
Phase 187 is part of milestone 20, External Review And Conclusion Publication. The roadmap requires closure for release trust boundary without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: threat_model_v2, tcb_review_report_v2, mitigation_checklist, residual_risk_acceptance. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
