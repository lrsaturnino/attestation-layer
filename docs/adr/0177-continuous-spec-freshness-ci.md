...
Phase 168 is part of milestone 17, Brownfield Spec Grounding And Drift Closure. The roadmap requires closure for stale-spec branch blocking without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: freshness_ci_report_v2, changed_code_detection_report, validation_recency_report, waiver_audit_record. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
