...
Phase 173 is part of milestone 18, Production Adapter And Trace Closure. The roadmap requires closure for transaction/event ecosystem evidence without relying on shallow fixtures or implicit review assumptions.

## Decision

Represent the phase as a schema-backed real-evidence phase report. The required artifact types are: solidity_adapter_v3_report, evm_trace_producer_report, solidity_impact_fixture, solidity_limitation_report. Each artifact must be accepted, reviewed, replayable, and explicitly marked as real evidence before the phase can pass.
